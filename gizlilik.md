# Gizlilik politikası

**Edebiyat Atlası**

Son güncelleme: 30 Temmuz 2026

---

## 1. Veri sorumlusu

Bu uygulamayı geliştiren ve verilerinizin işlenmesinden sorumlu olan taraf:

- **Veri sorumlusu:** Faruk Okuyucu
- **İletişim:** okuyucufa@gmail.com

Gizlilikle ilgili her soru ve talep için bu adrese yazabilirsiniz.

## 2. Hangi verileri işliyoruz?

### a) Google hesabınızdan gelen bilgiler

Uygulamaya yalnızca Google hesabınızla giriş yapabilirsiniz. Giriş sırasında
Google'dan şu bilgiler alınır:

- **E-posta adresiniz**
- **Adınız** (Google hesabınızdaki görünen ad)
- **Profil fotoğrafınızın adresi** (fotoğrafın kendisi değil, Google'daki bağlantısı)

Google şifreniz bize hiçbir zaman ulaşmaz; girişi Google kendi sayfasında
gerçekleştirir.

### b) Uygulamayı kullanırken oluşan bilgiler

- **Test kayıtları:** çözdüğünüz her testin türü (günlük pratik, hızlı test,
  dönem testi), soru sayısı, doğru sayısı, kazanılan XP, testin sürdüğü süre ve
  tarihi
- **Soru bazında sonuçlar:** hangi soruyu doğru, hangisini yanlış cevapladığınız
- **Flashcard ilerlemesi:** hangi kartı "biliyorum" ya da "bilemedim" olarak
  işaretlediğiniz
- **İlerleme özetleri:** toplam XP, güncel seri, en uzun seri, son çalıştığınız
  gün ve en son çalıştığınız dönem

Bu kayıtlar ilerlemenizi hesaplamak ve size henüz bilmediğiniz soruları öncelikli
göstermek için tutulur.

## 3. Hangi verileri İŞLEMİYORUZ?

Bunları açıkça belirtmek istiyoruz:

- **Reklam yok, reklam kimliği kullanılmıyor.**
- **Analitik veya kullanıcı izleme aracı yok.** Uygulamada hiçbir analitik,
  reklam veya izleme yazılımı bulunmuyor.
- **Çökme raporlama aracı yok.**
- **Konumunuza erişilmiyor.**
- **Rehberinize, fotoğraflarınıza, kameranıza ve mikrofonunuza erişilmiyor.**
- **Bildirim gönderilmiyor.**
- **Verileriniz hiç kimseye satılmıyor, kiralanmıyor, reklam amacıyla
  paylaşılmıyor.**
- **Cihaz kimliğiniz, IMEI, MAC adresi gibi donanım bilgileri toplanmıyor.**

## 4. Cihazınızda saklananlar

Aşağıdakiler yalnızca telefonunuzda tutulur, bize gönderilmez:

- **Oturum bilgisi:** her açılışta yeniden giriş yapmanız gerekmesin diye
- **Günün sorusu ve günlük pratik turu:** o günün sorularını ve verdiğiniz
  cevapları, gün içinde kaldığınız yerden devam edebilmeniz için

Uygulamayı telefonunuzdan silerseniz bu veriler de silinir.

## 5. İşleme amaçları ve hukuki dayanak

| Amaç | Hukuki dayanak (KVKK m.5) |
| --- | --- |
| Hesabınızı oluşturmak ve girişinizi sağlamak | Sözleşmenin kurulması ve ifası |
| İlerlemenizi, XP ve serinizi hesaplayıp göstermek | Sözleşmenin ifası |
| Size henüz bilmediğiniz soruları öncelikli göstermek | Sözleşmenin ifası |
| Hesabınızı silme talebinizi yerine getirmek | Hukuki yükümlülük / açık talebiniz |

Verileriniz bu amaçların dışında kullanılmaz. Profilleme yoluyla hakkınızda
otomatik bir karar alınmaz.

## 6. Verilerin saklandığı yer ve üçüncü taraflar

Verileriniz iki hizmet sağlayıcı üzerinden işlenir:

- **Google LLC (ABD)** — yalnızca giriş işlemi için. Google, uygulamaya giriş
  yaptığınızı görür; uygulama içindeki çalışmanızı görmez.
- **Supabase Inc.** — hesabınız ve ilerleme kayıtlarınız Supabase'in yönettiği
  veritabanında barındırılır. Supabase, bizim adımıza veri işleyen taraftır.

Bunların dışında hiçbir üçüncü tarafa veri aktarılmaz.

### Verilerin yurt dışına aktarılması

Hesabınız ve ilerleme kayıtlarınız Supabase'in **Sidney (Avustralya)**
bölgesindeki sunucularında (`ap-southeast-2`) barındırılmaktadır. Bu, kişisel
verilerinizin **Türkiye dışına aktarıldığı** anlamına gelir. Google girişi
nedeniyle e-posta ve ad bilginiz ayrıca Google LLC (ABD) tarafından da işlenir.

KVKK m.9 uyarınca kişisel verilerin yurt dışına aktarılması, Kişisel Verileri
Koruma Kurulu'nun yeterlilik kararı bulunan ülkelere ya da uygun güvencelerin
sağlanması hâlinde mümkündür. **Avustralya hakkında verilmiş bir yeterlilik kararı
bulunmamaktadır.** Aktarım, uygulamayı kullanmaya başlarken bu politikayı kabul
ederek verdiğiniz açık rızaya dayanılarak gerçekleştirilir.

Verilerinizin yurt dışına aktarılmasını istemiyorsanız uygulamayı kullanmamanız,
hesabınız varsa silmeniz gerekir. Rızanızı her zaman geri alabilirsiniz; bunun
yolu hesabınızı silmektir (bkz. 7. bölüm).

## 7. Saklama süresi ve silme

Verileriniz hesabınız var olduğu sürece saklanır.

**Hesabınızı uygulama içinden kendiniz silebilirsiniz:** Ana Sayfa → sağ üstteki
hesap simgesi → **Hesabı Sil**. Onayladığınızda hesabınız ve ona bağlı **bütün**
veri (profil, test kayıtları, soru sonuçları, flashcard ilerlemesi) veritabanından
kalıcı olarak silinir. Bu işlem geri alınamaz ve silinen verinin yedeği tutulmaz.

Silme talebinizi okuyucufa@gmail.com adresine yazarak da iletebilirsiniz.

## 8. Haklarınız

KVKK m.11 uyarınca; verilerinizin işlenip işlenmediğini öğrenme, işlenmişse buna
ilişkin bilgi talep etme, işlenme amacını öğrenme, aktarıldığı tarafları bilme,
eksik veya yanlış işlenmişse düzeltilmesini isteme, silinmesini veya yok
edilmesini isteme, bu işlemlerin aktarılan taraflara bildirilmesini isteme,
işlenen verilere itiraz etme ve zarara uğramanız hâlinde giderilmesini talep etme
haklarına sahipsiniz.

Avrupa Birliği'nde bulunuyorsanız GDPR kapsamında erişim, düzeltme, silme,
işlemenin kısıtlanması, veri taşınabilirliği ve itiraz haklarınız vardır.

Taleplerinizi okuyucufa@gmail.com adresine iletebilirsiniz; en geç 30 gün içinde
yanıtlanır.

## 9. Çocukların verileri

Uygulama sınavlara hazırlanan kullanıcılar için tasarlanmıştır ve kullanıcıların
bir bölümü 18 yaşın altında olabilir. Bilerek 13 yaşın altındaki kullanıcılardan
veri toplanmaz. Velisi olduğunuz bir çocuğun verisinin silinmesini istiyorsanız
okuyucufa@gmail.com adresine yazın, hesabı siliyoruz.

## 10. Değişiklikler

Bu politika değişirse yukarıdaki "Son güncelleme" tarihi güncellenir. Önemli bir
değişiklik olduğunda uygulama içinde bilgilendirme yapılır.

## 11. İletişim

Faruk Okuyucu — okuyucufa@gmail.com
